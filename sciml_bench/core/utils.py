...
def homogenize_mpi_env():
    """
    Checks for Hydra environment variables; if present, copy them to the same ones used
    more generally by PyTorch for consistency.
    """
    for a, b in zip(["PMI_SIZE", "PMI_RANK"], ["WORLD_SIZE", "LOCAL_RANK"]):
        if a in os.environ:
            # this copies over MPI variables to NCCL ones for consistency
