...
def sciml_bench_inference(params_in: RuntimeIn, params_out: RuntimeOut):
    ...
    # Load the model and move it to the right device and optimize with IPEX
    with log.subproc(f'Loading the model for inference into {device}, and performing IPEX optimizations'):
        model = torch.load(params_in.model)
        model.to(device)
