...
from sciml_bench.benchmarks.examples.mnist_torch_horovod.impl_mnist_torch \
...
def sciml_bench_training(smlb_in: RuntimeIn, smlb_out: RuntimeOut):
    ...
    default_args = {
        # torch env
        'seed': 1234, 'use_xpu': True,
        # network parameters
        'n_filters': 32, 'n_unit_hidden': 128,
        # hyperparameters
        'epochs': 10, 'batch_size': 64, 'loss_func': 'CrossEntropyLoss',
    ...
    if args['use_xpu']:
        if torch.xpu.is_available():
            ...
            console.message('XPU: {} available.'.format(device))
            # Horovod: limit # of CPU threads to be used per worker
            torch.set_num_threads(1)
        else:
            args['use_xpu'] = False
            console.message('WARNING: XPU is unavailable, '
                            'setting argument "use_xpu" to False')
    ...
    with console.subproc(f'Warmup steps'):
        history = train(model,
                        train_sampler, train_loader, test_sampler, test_loader,
                        use_xpu=args['use_xpu'], epochs=1,
                        loss_func=args['loss_func'],
                        optimizer_name=args['optimizer_name'], lr=args['lr'],
                        batch_log_interval=args['batch_log_interval'],
                        hvd=hvd, smlb_out=smlb_out, xpu=device)

    # Start time tracking after warming up the model
    console.begin('Running benchmark mnist_torch_horovod')
    ...
    if args['load_weights_file'] == '':
        ...
        history = train(model,
                        train_sampler, train_loader, test_sampler, test_loader,
                        use_xpu=args['use_xpu'], epochs=args['epochs'],
                        loss_func=args['loss_func'],
                        optimizer_name=args['optimizer_name'], lr=args['lr'],
                        batch_log_interval=args['batch_log_interval'],
                        hvd=hvd, smlb_out=smlb_out, xpu=device)
    ...
    if hvd.rank() == 0:
        ...
        with console.subproc('Making predictions on test set (on root only)'):
            pred = predict(model, test_set.images, args['use_xpu'], device,
                           to_classes=True)
    ...
    # top-level process
    console.ended('Running benchmark mnist_torch_horovod')
